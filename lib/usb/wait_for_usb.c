...
size_t write_to_jump(const char* mnt_point, const char* filename) {
	...
	char* file_dest = (char*)malloc(256);
	if (file_src == NULL) {
...
bool wait_for_usb(const char* filename) {
	...
	flag_e = flag_p = (cap_flag_value_t)NULL;
...
**/
					char* partnum = (char*)malloc(3);
					char* partname = (char*)malloc(10);
...
//									printf("WAITUSB :: mount() :: EBUSY --> %s %s\n", ent->mnt_fsname, ent->mnt_dir);
					...
					if (mounted) {
						int res = 0;
						res = write_to_jump(target, filename);
